...
func (uq *UserQuery) QueryEvaluations() *EvaluationQuery {
	query := (&EvaluationClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withEvaluations: uq.withEvaluations.Clone(),
...
func (uq *UserQuery) WithEvaluations(opts ...func(*EvaluationQuery)) *UserQuery {
	query := (&EvaluationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withEvaluations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withEvaluations; query != nil {
		if err := uq.loadEvaluations(ctx, query, nodes,
			func(n *User) { n.Edges.Evaluations = []*Evaluation{} },
			func(n *User, e *Evaluation) { n.Edges.Evaluations = append(n.Edges.Evaluations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadEvaluations(ctx context.Context, query *EvaluationQuery, nodes []*User, init func(*User), assign func(*User, *Evaluation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(evaluation.FieldUserId)
	}
	query.Where(predicate.Evaluation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EvaluationsColumn), fks...))
	}))
