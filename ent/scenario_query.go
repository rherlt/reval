...
func (sq *ScenarioQuery) QueryResponses() *ResponseQuery {
...
func (sq *ScenarioQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (sq *ScenarioQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (sq *ScenarioQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (sq *ScenarioQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (sq *ScenarioQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (sq *ScenarioQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *ScenarioQuery) WithResponses(opts ...func(*ResponseQuery)) *ScenarioQuery {
	query := (&ResponseClient{config: sq.config}).Query()
...
func (sq *ScenarioQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Scenario, error) {
	...
	if query := sq.withResponses; query != nil {
		if err := sq.loadResponses(ctx, query, nodes,
			func(n *Scenario) { n.Edges.Responses = []*Response{} },
			func(n *Scenario, e *Response) { n.Edges.Responses = append(n.Edges.Responses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ScenarioQuery) loadResponses(ctx context.Context, query *ResponseQuery, nodes []*Scenario, init func(*Scenario), assign func(*Scenario, *Response)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Scenario)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(response.FieldScenarioId)
	}
	query.Where(predicate.Response(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(scenario.ResponsesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "scenarioId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (sq *ScenarioQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(scenario.Table, scenario.Columns, sqlgraph.NewFieldSpec(scenario.FieldID, field.TypeUUID))
	_spec.From = sq.sql
